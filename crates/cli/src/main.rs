...
mod tests {
    ...
    fn cross_invocation_demo_snapshot_proves_readback_across_process_invocations() {
        // Integration test: run the demo with snapshot, then read back in a
        // separate process invocation.  Proves the governed FailureInsight
        // learning loop output survives cross-process readback.
        ...
        // Locate the built binary relative to the crate manifest dir.
        let manifest_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let bin_path = manifest_dir.join("../../target/debug/arpagona");

        // Step 1: run demo with --snapshot-path to produce the snapshot file
        let output1 = std::process::Command::new(&bin_path)
            .args([
                "memory",
                "demo",
                "failure-insight",
